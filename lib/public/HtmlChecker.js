...
function excludeLink(link, instance)
{
	...
	// use includedKeywords to include links. Undocumented filter doesn't work for me
	if (instance.options.includedKeywords.length > 0) {
		if (matchUrl(link.url.resolved, instance.options.includedKeywords) === false) return "BLC_KEYWORD";
	}
